...
def getNews(base, postfixs):
    ...
    for postfix in postfixs:
        ...
        if len(soup.select('h1')) > 0:
            ...
            sub_title = soup.select('.arti-update')
            text_arr = []
            for i in range(0, len(sub_title) - 1):
                text_arr.append(sub_title[i].get_text())
            subtitle = ''.join(text_arr)
            insertData('soaa', ['zh', title, subtitle, base+postfix])
            current_count = current_count + 1


if __name__ == '__main__':
    for i in range(1, 11):
        urls = getUrls("http://www.soaa.zju.edu.cn/31331/list" + str(i) + (".htm" if i <= 5 else ".psp"))
        base = 'http://www.soaa.zju.edu.cn'
        getNews(base, urls)

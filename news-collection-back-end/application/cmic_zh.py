...
def getNews(base, postfixs):
    ...
    for postfix in postfixs:
        ...
        soup = BeautifulSoup(strhtml.text, 'lxml')
        if len(soup.select('#contentcn>.art-heading')) > 0:
            title = soup.select('#contentcn>.art-heading')[0].get_text()
            sub_title = soup.select('#contentcn>.art-summary')
            text_arr = []
            for i in range(0, len(sub_title) - 1):
                text_arr.append(sub_title[i].get_text())
            subtitle = ''.join(text_arr)
            insertData('cmic', ['zh', title, subtitle, base+postfix])
            current_count = current_count + 1


if __name__ == '__main__':
    for i in range(1, 236):
        urls = getUrls("http://www.cmic.zju.edu.cn/35554/list" + str(i) + (".htm" if i <= 5 else ".psp"))
